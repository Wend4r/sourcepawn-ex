...
EnumDecl::Bind()
{
	...
	if(name_)
	{
		...
		if(!enumsym)
		{
			...
			if((enumsym = add_constant(name_->chars(), 0, vclass_, tag)))
			{
...
TypedefDecl::Bind()
{
	...
	functags_add_or_find(funcenums_add_or_find(name_->chars()), type_);
...
UsingDecl::Bind()
{
	declare_handle_intrinsics(this->name()->chars());
...
TypesetDecl::Bind()
{
	...
	funcenum_t* def = funcenums_add_or_find(name_->chars());

	for(const auto& type : types_)
	{
		functags_add_or_find(def, type);
	}
