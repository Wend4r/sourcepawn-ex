...
#define sDEF_ADDDYNAMICSTACK 512 /* default stack size for AMX files */
...
struct constvalue
{
	...
	char name[sNAMEMAX + 1];
	cell value;
	int index; /* index level, for constants referring to array sizes/tags tag for enumeration lists */
};
...
struct symbol
{
	...
	uint16_t fnumber; // File number in which the symbol is declared.

	uint32_t lnumber; // Line number for the declaration.
	uint16_t colnumber; /*  */
...
private:
	int32_t addr_; // Address or offset (or value for constant, index for native function).
	sp::Atom* name_;
...
#define uREAD       (1 << 0)               // Used/accessed.
#define uWRITTEN    (1 << 1)               // Altered/written (variables only).
#define uROOT       (uREAD | uWRITTEN)     // All access.
...
struct value
{
	char ident;      // iCONSTEXPR, iVARIABLE, iARRAY, iARRAYCELL, iEXPRESSION or iREFERENCE.
	symbol* sym;     // Symbol in symbol table, NULL for(constant) expression.
	cell constval;   // Value of the constant expression (if ident==iCONSTEXPR). Also used for the size of a literal array.
...
size_t info_to_type_definition(int iIdent, int iTag, const int iArrayLength[sDIMEN_MAX], int iArrayLevel, bool bIsConst, char *psTypeDef, size_t nMaxLength);
size_t symbol_to_type_definition(const symbol *pSymbol, char *psTypeDef, size_t nMaxLength);
size_t value_to_type_definition(const value *pValue, char *psTypeDef, size_t nMaxLength);
size_t arginfo_to_type_definition(const arginfo *pArgInfo, char *psTypeDef, size_t nMaxLength);
size_t funcarg_to_type_definition(const funcarg_t *pFuncArg, char *psTypeDef, size_t nMaxLength);
size_t functag_args_to_definition(const functag_t *pFuncTag, char *psArgDef, size_t nMaxLength);
size_t functag_info_to_definition(const Type *pFunction, char *psFuncTagDef, size_t nMaxLength);
